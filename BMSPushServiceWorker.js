...
function displayNotification(event) {
  // data format: {"tag":"Notifications-VJ","messageId":"d5uZ2Seu","alert":"IBM CEC sample notification","payload":{"alert":"IBM CEC sample notification","url":"www.ibm.com"}}
  try {
...
function onPushNotificationReceived(event) {
    ...
    if (typeof appendNotification === "function") {
      appendNotification('info', 'Push notification received :' + event.data.text());
    }
...
self.addEventListener('install', function (event) {
    ...
    if (typeof appendlogs === "function") {
      appendlogs('info', 'Installed Service Worker : '+ event.data.text());
    }
...
self.addEventListener('activate', function (event) {
    ...
    if (typeof appendlogs === "function") {
      appendlogs('info', 'Activated Service Worker : '+ event.data.text());
    }
...
self.addEventListener('notificationclick', function (event) {
    ...
    if (typeof appendlogs === "function") {
      appendlogs('info', 'Notification clicked with tag' + event.notification.tag + " and data " + event.notification.data);
    }
